...
pub fn accept_selected_completion_candidate(code_editor: &mut CodeEditor) {
    ...
    if let Some(candidate) = code_editor
        .completion_state
        .candidates
        .get(code_editor.completion_state.selected_index)
    {
        // 現在のコンテンツを取得
        let current_content =
        ...
        // プレフィックスを削除
        let prefix_start = code_editor
            .completion_state
            .trigger_position
            .saturating_sub(code_editor.completion_state.prefix.len());
...
pub fn trigger_code_completion_for_current_position(code_editor: &mut CodeEditor) {
    ...
    let language = if let Some(active_file) = code_editor.open_files.get(code_editor.active_tab) {
        crate::ui::code_editor::syntax_highlight::detect_programming_language_from_file_path(
            &active_file.path,
        )
    ...
    let candidates = crate::ui::code_completion::get_completion_candidates(
        &prefix, language, content, cursor_pos,
    );
