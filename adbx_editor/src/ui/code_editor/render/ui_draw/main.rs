...
pub fn draw_code_editor_ui(
    ...
    if let Some(panel) = panel_opt {
        ...
        if needs_update {
            ...
            // コンテンツの子要素を追加
            commands.entity(content_entity_id).with_children(
            ...
            commands
                ...
                .add_child(editor_area_entity);

            // エディタエリアの子要素を追加
            commands.entity(editor_area_entity).with_children(
                |editor_area: &mut ChildSpawnerCommands| {
            ...
            if code_editor.completion_state.is_visible
                && !code_editor.completion_state.candidates.is_empty()
            {
                crate::ui::code_editor_completion::draw_completion_popup(
                    &mut commands,
                    &code_editor.completion_state,
                    editor_area_entity,
                );
            }
