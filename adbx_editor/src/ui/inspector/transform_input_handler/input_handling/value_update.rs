    ...
    if let Ok(mut transform) = transform_query.get_mut(entity) {
        ...
        match field_type {
            ...
            TransformFieldType::RotationX => {
                ...
                transform.rotation =
                    Quat::from_euler(bevy::math::EulerRot::XYZ, new_roll, pitch, yaw);
            }
            TransformFieldType::RotationY => {
                let (roll, pitch, yaw) = transform.rotation.to_euler(bevy::math::EulerRot::XYZ);
                let new_pitch = (pitch.to_degrees() + normalized_delta).to_radians();
                transform.rotation =
                    Quat::from_euler(bevy::math::EulerRot::XYZ, roll, new_pitch, yaw);
            }
            TransformFieldType::RotationZ => {
                let (roll, pitch, yaw) = transform.rotation.to_euler(bevy::math::EulerRot::XYZ);
                let new_yaw = (yaw.to_degrees() + normalized_delta).to_radians();
                transform.rotation =
                    Quat::from_euler(bevy::math::EulerRot::XYZ, roll, pitch, new_yaw);
            }
            TransformFieldType::ScaleX => {
                transform.scale.x =
                    normalize_transform_value(field_type, transform.scale.x + normalized_delta);
            }
            TransformFieldType::ScaleY => {
                transform.scale.y =
                    normalize_transform_value(field_type, transform.scale.y + normalized_delta);
            }
            TransformFieldType::ScaleZ => {
                transform.scale.z =
                    normalize_transform_value(field_type, transform.scale.z + normalized_delta);
            }
        }
    }
}
