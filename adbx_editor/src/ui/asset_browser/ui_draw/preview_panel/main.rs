...
use super::generic_preview::{draw_generic_preview, draw_mesh_preview};
...
fn draw_preview_content(
    ...
    let asset_file_info = asset_browser
        .asset_files
        .iter()
    ...
    if let Some((asset_file_path, asset_file_name, asset_file_type)) = asset_file_info {
        ...
        let image_handle_opt = if asset_file_type == AssetType::Texture {
            let image_path_str = if asset_file_path.starts_with("assets/") {
                ...
                asset_file_path.to_str().map(|s| s.replace('\\', "/"))
            };
            ...
            if let Some(image_path) = image_path_str {
                Some(
                    asset_browser
                        ...
                        .or_insert_with(|| asset_server.load::<Image>(&image_path))
                        .clone(),
                )
            } else {
                None
        ...
        // 画像が読み込まれているかチェック
        let image_loaded = image_handle_opt
            .as_ref()
