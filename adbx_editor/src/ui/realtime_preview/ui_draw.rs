...
pub fn draw_realtime_preview(
    ...
    if let Some((file_path, content)) = current_file {
        ...
        // プレビューを更新
        update_preview(&mut preview, file_path.clone(), content.clone(), preview_type);
        ...
        if window_opt.is_none() && preview.preview_window_entity.is_none() {
            if let Some(window) = windows.iter().next() {
                ...
                let preview_window = commands.spawn((
                    Node {
                        width: Val::Px(400.0),
                        height: Val::Px(300.0),
                            ...
                            preview_window.spawn((
                                ...
                                if let Some(ref rendered) = preview_content.rendered_content {
                                    ...
                                    if let Some(ref error) = preview_content.error {
                                        ...
                                        content.spawn((
                                            Text::new("Preview not available for this file type"),
                                            bevy::text::TextFont {
                                                font_size: 10.0,
                                                ..default()
                                            },
