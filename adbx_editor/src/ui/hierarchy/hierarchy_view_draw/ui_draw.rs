use super::super::hierarchy_view_resource::{HierarchyDragState, HierarchyItem, HierarchyView};
use super::entity_collection::{
...
pub fn draw_hierarchy_view_ui(
    ...
    if let Some(panel) = panel_opt {
        ...
        let visible_entities = collect_expanded_hierarchy_entities(
            &root_entities,
            &hierarchy_view,
            &children_query,
            &name_query,
        ...
        // スクロール位置に基づいて表示範囲を計算
        let scroll_index =
            (hierarchy_view.scroll_offset / hierarchy_view.item_height).floor() as usize;
        ...
        let removed_entities: Vec<Entity> = hierarchy_view
            .last_entities
            .difference(&current_entities)
            .copied()
            .collect();
        let _new_entities: Vec<Entity> = current_entities
            .difference(&hierarchy_view.last_entities)
            .copied()
            .collect();
        ...
        hierarchy_view
            .ui_entity_map
            .retain(|entity, _| visible_range_entities.contains(entity));
        ...
        for (index, entity) in visible_entities[start_index..end_index].iter().enumerate() {
            if !hierarchy_view.ui_entity_map.contains_key(entity) {
                if let Ok(name) = name_query.get(*entity) {
                    ...
                    let indent = calculate_hierarchy_item_indent_level(
                        *entity,
                        &root_entities,
                        &children_query,
                        &hierarchy_view,
