use bevy::prelude::*;
use crate::ui::hierarchy::hierarchy_view_resource::{HierarchyView, HierarchyDragState, HierarchyItem};
            ...
            for (interaction, hierarchy_item) in interaction_query.iter() {
                if *interaction == Interaction::None {
                    if children_query.get(hierarchy_item.entity).is_ok() {
                        if hierarchy_view.expanded_entities.contains(&hierarchy_item.entity) {
                            hierarchy_view.expanded_entities.remove(&hierarchy_item.entity);
                        } else {
                            hierarchy_view.expanded_entities.insert(hierarchy_item.entity);
                        }
                    }
                }
            }
