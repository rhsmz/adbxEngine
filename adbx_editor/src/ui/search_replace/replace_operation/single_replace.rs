...
pub fn perform_replace(
    ...
    // 現在の結果を置換
    if let Some(result) = search_replace.search_results.get(search_replace.current_result_index) {
        // 現在のファイルの内容を取得
        ...
        if let Some(line) = lines.get(result.line) {
            ...
            *current_content = new_lines.join("\n");
            ...
            // 次の結果に移動
            search_replace.current_result_index = (search_replace.current_result_index + 1) % search_replace.search_results.len();
        }
    }
}
