...
pub fn perform_replace_all(
    ...
    if search_replace.use_regex {
        ...
        if let Ok(re) = regex::Regex::new(&search_replace.search_text) {
            *current_content = re.replace_all(current_content, &search_replace.replace_text).to_string();
        }
    } else {
        // 通常の文字列置換
        *current_content = current_content.replace(&search_replace.search_text, &search_replace.replace_text);
    }
