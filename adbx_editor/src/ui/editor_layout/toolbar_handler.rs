        ...
        for (interaction, name) in interaction_query.iter() {
            if *interaction == Interaction::Pressed {
                ...
                if name_str == "ToolbarNewFile" {
                    ...
                    code_editor
                        .open_files
                        .push(crate::ui::code_editor::OpenFile {
                ...
                if name_str == "ToolbarSave" {
                    crate::ui::code_editor::save_active_file_in_code_editor(
                        &mut *code_editor,
                        &mut *error_dialog,
                    );
                }
                ...
                if name_str == "ToolbarPlay" {
                    if runtime_state.is_stopped() {
                        // ランタイムプロセスを起動
                        let project_path: Option<&std::path::Path> =
                ...
                if name_str == "ToolbarStop" {
                    if runtime_state.is_playing() || runtime_state.is_paused() {
                        // ランタイムプロセスを停止
                        if let Err(e) =
                            crate::communication::stop_runtime_process(&mut communication)
                        {
    ...
    if let Some(result) = file_dialog_request.result.take() {
        match result {
            crate::ui::file_dialog::FileDialogResult::File(path) => {
                ...
                crate::ui::code_editor::open_file_in_code_editor(
                    path_str,
                    &mut *code_editor,
                    &mut *error_dialog,
                );
