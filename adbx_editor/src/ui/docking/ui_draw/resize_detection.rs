...
pub fn detect_resize_edge(
    ...
    let node_size = match (node.width, node.height) {
        ...
        (Val::Percent(w), Val::Percent(h)) => {
            Vec2::new(window.width() * w / 100.0, window.height() * h / 100.0)
        }
        (Val::Px(w), Val::Percent(h)) => Vec2::new(w, window.height() * h / 100.0),
        (Val::Percent(w), Val::Px(h)) => Vec2::new(window.width() * w / 100.0, h),
        _ => Vec2::ZERO,
    };
