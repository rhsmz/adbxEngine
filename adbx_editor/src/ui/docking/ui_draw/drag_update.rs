use super::super::drag_handler::{detect_drop_zone, save_layout, DropZoneIndicator};
...
pub fn update_panel_drag(
    ...
    if let Some(ref mut drag_state) = docking.drag_state {
        if mouse_input.pressed(MouseButton::Left) {
            ...
            let _ = drag_state; // drag_stateの借用を終了
                    ...
                    if let Some(edge) = resize_edge {
                        ...
                        match edge {
                            ResizeEdge::Left | ResizeEdge::Right => {
                                panel_state.size.0 =
                                    (panel_state.size.0 + delta.x).max(100.0).min(800.0);
                            }
                            ResizeEdge::Top | ResizeEdge::Bottom => {
                                panel_state.size.1 =
                                    (panel_state.size.1 + delta.y).max(100.0).min(600.0);
                            }
                        }
