...
/// 選択範囲をコピー
pub fn copy_selection(code_editor: &mut CodeEditor, text_editor_state: &mut TextEditorState) {
...
/// テキストをペースト
pub fn paste_text(code_editor: &mut CodeEditor, text_editor_state: &mut TextEditorState) {
    #[cfg(not(target_arch = "wasm32"))]
    {
        if let Ok(mut clipboard) = arboard::Clipboard::new() {
...
/// 選択範囲をカット
pub fn cut_selection(code_editor: &mut CodeEditor, text_editor_state: &mut TextEditorState) {
