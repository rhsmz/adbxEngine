...
use crate::ui::scene_view::{SceneView, GizmoMode};
    ...
    if let Some(&entity) = selection.selected_entities.first() {
        if let Ok(transform) = transform_query.get(entity) {
            match scene_view.gizmo_mode {
                GizmoMode::Translate => {
                    return detect_translate_handle(ray.origin, ray.direction.normalize(), transform, &camera_query);
                }
                GizmoMode::Rotate => {
                    return detect_rotate_handle(ray.origin, ray.direction.normalize(), transform, camera_transform, &camera_query);
                }
                GizmoMode::Scale => {
                    return detect_scale_handle(ray.origin, ray.direction.normalize(), transform, &camera_query);
                }
                _ => {}
            }
        }
