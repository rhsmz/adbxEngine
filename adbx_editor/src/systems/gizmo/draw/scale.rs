...
pub fn draw_scale_gizmo(
    ...
    gizmos.cuboid(
        Transform::from_translation(x_end).with_scale(Vec3::splat(handle_size)),
        x_color,
    );
    ...
    gizmos.cuboid(
        Transform::from_translation(y_end).with_scale(Vec3::splat(handle_size)),
        y_color,
    );
    ...
    gizmos.cuboid(
        Transform::from_translation(z_end).with_scale(Vec3::splat(handle_size)),
        z_color,
    );
}
