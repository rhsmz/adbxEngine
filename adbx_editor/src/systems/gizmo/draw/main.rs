    ...
    for &entity in selection.selected_entities.iter() {
        if let Ok(transform) = transform_query.get(entity) {
            match scene_view.gizmo_mode {
                GizmoMode::Translate => {
                    draw_translate_gizmo(&mut gizmos, transform, &gizmo_interaction, &camera_query, &windows);
                }
                GizmoMode::Rotate => {
                    draw_rotate_gizmo(&mut gizmos, transform, &gizmo_interaction, &camera_query, &windows);
                }
                GizmoMode::Scale => {
                    draw_scale_gizmo(&mut gizmos, transform, &gizmo_interaction, &camera_query, &windows);
                }
                GizmoMode::None => {
                    // 基本的な軸表示のみ
