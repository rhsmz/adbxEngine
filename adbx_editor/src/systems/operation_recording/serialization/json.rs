...
pub fn get_history_json(recorder: &OperationRecorder, limit: Option<usize>) -> Result<String, String> {
    let limit = limit.unwrap_or(recorder.operations.len());
    let operations: Vec<&super::super::RecordedOperation> = recorder.operations
        .iter()
        .rev()
        .take(limit)
        .collect();
