...
fn ray_aabb_intersection(
    ...
    let inv_dir = Vec3::new(
        if ray_direction.x.abs() > epsilon { 1.0 / ray_direction.x } else { f32::MAX },
        if ray_direction.y.abs() > epsilon { 1.0 / ray_direction.y } else { f32::MAX },
        if ray_direction.z.abs() > epsilon { 1.0 / ray_direction.z } else { f32::MAX },
    );
        ...
        if let Some(cursor_position) = window.cursor_position() {
            ...
            if let Ok(ray) = camera.viewport_to_world(camera_transform, cursor_position) {
                ...
                for (entity, transform) in transform_query.iter() {
                    // AABBとの交差判定
                    if let Some(distance) = ray_transform_intersection(
                        ray.origin,
                ...
                if selection.selected_entities != previous_selection {
                    communication::notify_entity_selection(&*communication, &selection.selected_entities);
                }
            }
        }
    }
