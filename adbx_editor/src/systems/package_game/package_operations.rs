...
/// ゲームをパッケージ化する
pub fn package_game(project_path: &PathBuf, build_output_path: &PathBuf) -> Result<(), String> {
    // パッケージディレクトリを作成
    ...
    fs::copy(&exe_source, &exe_dest).map_err(|e| format!("Failed to copy executable: {}", e))?;
